mdes.cra4r4 <- function(power=.80, alpha=.05, two.tail=TRUE,
  ...
  mdes = M*sqrt(rho4*(1-R42)/(P*(1-P)*L) +
  ...
  par <- list(power=power, alpha=alpha, two.tail=two.tail,
              ...
              P=P, g4=g4, R12=R12, R22=R22, R32=R32, R42=R42,
              n=n, J=J, K=K, L=L)
...
power.cra4r4 <- function(mdes=.25, alpha=.05, two.tail=TRUE,
                         ...
                         n, J, K, L, ...){
  df = L-g4-2
  lamda <- mdes/sqrt(rho4*(1-R42)/(P*(1-P)*L) +
                     rho3*(1-R32)/(P*(1-P)*K*L) +
                     rho2*(1-R22)/(P*(1-P)*J*K*L) +
                    (1-rho4-rho3-rho2)*(1-R12)/(P*(1-P)*J*K*L*n))
...
mrss.cra4r4 <- function(mdes=.25, power=.80, alpha=.05, two.tail=TRUE,
  ...
  eq.mdes <- function(nJKL){
    ...
    T2 = abs(qt(power,df))
    M <- ifelse(power>=.5,T1+T2,T1-T2)
    mdes1 = M*sqrt(rho4*(1-R42)/(P*(1-P)*L) +
                   rho3*(1-R32)/(P*(1-P)*K*L) +
                   rho2*(1-R22)/(P*(1-P)*J*K*L) +
                  (1-rho4-rho3-rho2)*(1-R12)/(P*(1-P)*J*K*L*n))
    mdes0 <-mdes1-mdes
  ...
  if(is.null(L)){
    ...
    T2 = abs(qt(power,df))
    M <- ifelse(power>=.5,T1+T2,T1-T2)
    K <- (rho3*(1-R32) + rho2*(1-R22)/J +
         (1-rho4-rho3-rho2)*(1-R12)/(J*n))/
         (L*P*(1-P)*(mdes/M)^2 - rho4*(1-R42))
    K <- round(K)
  }else if(is.null(J)){
    df = L-g4-2
    T1 <- ifelse(two.tail==TRUE,abs(qt(alpha/2,df)),abs(qt(alpha,df)))
    T2 = abs(qt(power,df))
    M <- ifelse(power>=.5,T1+T2,T1-T2)
    J <- (n*rho2*(1-R22) + (1-rho4-rho3-rho2)*(1-R12))/
         (P*(1-P)*n*K*L*(mdes/M)^2 - n*K*rho4*(1-R42) - n*rho3*(1-R32))
    J <- round(J)
  }else if(is.null(n)){
    df = L-g4-2
    T1 <- ifelse(two.tail==TRUE,abs(qt(alpha/2,df)),abs(qt(alpha,df)))
    ...
    n <- (1-rho4-rho3-rho2)*(1-R12)/
         (P*(1-P)*J*K*L*(mdes/M)^2 - J*K*rho4*(1-R42) -
  ...
  gridnJKL <- expand.grid(c(nJKL1[1]-gm):(nJKL1[1]+gm),
                          c(nJKL1[2]-gm):(nJKL1[2]+gm),
                          c(nJKL1[3]-gm):(nJKL1[3]+gm),
                          c(nJKL1[4]-gm):(nJKL1[4]+gm))
...
# mrss.cra4r4(rho4=.05, rho3=.05, rho2=.10, n=10, J=2, L=51)
# mrss.cra4r4(rho4=.05, rho3=.05, rho2=.10, n=10, K=3, L=51)
# mrss.cra4r4(rho4=.05, rho3=.05, rho2=.10, J=2, K=3, L=51)
...
optimal.cra4r4 <- function(cn, cJ, cK, cL, cost=NULL, n=NULL, J=NULL, K=NULL, L=NULL,
                           power=.80, mdes=.25, alpha=.05, two.tail=TRUE,
                           nJKL0=c(10,10,10,10), ncase=10, gm=2, 
                           constrain="cost", optimizer="auglag_cobyla",
                           rho4, rho3, rho2, P=.50, g4=0, R42=0, R32=0, R22=0, R12=0){
...
# optimal.cra4r4(cn=1, cJ=10, cK=100, cL=1000, cost=75600, constrain="cost", rho4=.10, rho3=.15, rho2=.20)

# optimal sample given per unit costs and power 
# optimal.cra4r4(cn=1, cJ=10, cK=100, cL=1000, power=.80, constrain="power", rho4=.10, rho3=.15, rho2=.20)

# optimal sample given per unit costs and mdes
# optimal.cra4r4(cn=1, cJ=10, cK=100, cL=1000, power=.80, constrain="mdes", rho4=.10, rho3=.15, rho2=.20)

# conditional optimal sample (fixed n=10, and J=2) given total cost
# optimal.cra4r4(cn=1, cJ=10, cK=100, cL=1000, n=10, J=2, cost=75600, constrain="cost", rho4=.10, rho3=.15, rho2=.20)

# conditional optimal sample (fixed n=10, and J=2) given per unit costs and power 
# optimal.cra4r4(cn=1, cJ=10, cK=100, cL=1000, n=10, J=2, constrain="power", rho4=.10, rho3=.15, rho2=.20, nJKL0=c(10,2,3,3))

# conditional optimal sample (fixed n=10, and J=2) given per unit costs and mdes
# optimal.cra4r4(cn=1, cJ=10, cK=100, cL=1000, n=10, J=2, constrain="mdes", rho4=.10, rho3=.15, rho2=.20)
