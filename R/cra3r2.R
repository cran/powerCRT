mdes.cra3r2 <- function(power=.80, alpha=.05, two.tail=TRUE,
  ...
  par <- list(power=power, alpha=alpha, two.tail=two.tail,
              ...
              P=P, g3=g3, R12=R12, R22=R22, RT32=RT32,
              n=n, J=J, K=K)
  return(list(fun=fun,par=par,df=df,M=M,mdes=round(MLU,digits=3)))
}

# example
# mdes.cra3r2(rho3=.13, rho2=.10, omega3=.4, n=10, J=6, K=24)


power.cra3r2 <- function(mdes=.25, alpha=.05, two.tail=TRUE,
                         rho2, rho3,  omega3, P=.50, g3=0, R12=0, R22=0, RT32=0,
                         n, J, K, ...){
  df = K-g3-1
...
# example
# power.cra3r2(rho3=.13, rho2=.10, omega3=.4, n=10, J=6, K=24)


# gm = grid multiplier: default 2
# ncase = numer of fixed samples in the output
# constrain = "power", or "mdes" :  default "power"
mrss.cra3r2 <- function(mdes=.25, power=.80, alpha=.05, two.tail=TRUE,
  ...
  if(is.null(K)){
    ...
    J <- (rho2*(1-R22) + (1-rho3-rho2)*(1-R12)/n) /
         (P*(1-P)*K*(mdes/M)^2 - P*(1-P)*rho3*omega3*(1-RT32))
    J <- round(J)
  }else if(is.null(n)){
    df = K-g3-1
    T1 <- ifelse(two.tail==TRUE,abs(qt(alpha/2,df)),abs(qt(alpha,df)))
    T2 = abs(qt(power,df))
    M <- ifelse(power>=.5,T1+T2,T1-T2)
    n <- (1-rho3-rho2)*(1-R12)/ (P*(1-P)*J*K*(mdes/M)^2 -
...
# example
# mrss.cra3r2(rho3=.13, rho2=.10, omega3=.4, n=10, J=6)
# mrss.cra3r2(rho3=.13, rho2=.10, omega3=.4, n=10, K=23)
# mrss.cra3r2(rho3=.13, rho2=.10, omega3=.4, J=6, K=23)
...
optimal.cra3r2 <- function(cn, cJ, cK, cost=NULL, n=NULL, J=NULL, K=NULL,
...
# optimal.cra3r2(cn=1, cJ=10, cK=100, cost=5600, constrain="cost", rho3=.15, rho2=.20, omega3=.50)

# optimal sample given per unit costs and power
# optimal.cra3r2(cn=1, cJ=10, cK=100, power=.80, constrain="power", rho3=.15, rho2=.20, omega3=.50)

# optimal sample given per unit costs and mdes
# optimal.cra3r2(cn=1, cJ=10, cK=100, power=.80, constrain="mdes", rho3=.15, rho2=.20, omega3=.50)

# conditional optimal sample (fixed n=10) given total cost
# optimal.cra3r2(cn=1, cJ=10, cK=100, n=10, cost=5600, constrain="cost", rho3=.15, rho2=.20, omega3=.50)

# conditional optimal sample (fixed n=10) given per unit costs and power
# optimal.cra3r2(cn=1, cJ=10, cK=100, n=10, constrain="power", rho3=.15, rho2=.20, omega3=.50)

# conditional optimal sample (fixed n=10) given per unit costs and mdes
# optimal.cra3r2(cn=1, cJ=10, cK=100, n=10, constrain="mdes", rho3=.15, rho2=.20, omega3=.50)
