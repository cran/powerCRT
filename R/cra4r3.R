mdes.cra4r3 <- function(power=.80, alpha=.05, two.tail=TRUE,
  ...
  par <- list(power=power, alpha=alpha, two.tail=two.tail,
              ...
              P=P, g4=g4, R12=R12, R22=R22, R32=R32, RT42=RT42,
              n=n, J=J, K=K, L=L)
  return(list(fun=fun,par=par,df=df,M=M,mdes=round(MLU,digits=3)))
}

# example
# mdes.cra4r3(rho4=.05, rho3=.15, rho2=.15, omega4=.50, n=10, J=4, K=4, L=20)

power.cra4r3 <- function(mdes=.25, alpha=.05, two.tail=TRUE,
                         rho2, rho3, rho4, omega4, 
                         P=.50, R12=0, R22=0, R32=0, RT42=0, g4=0,
                         n, J, K, L, ...){
  df = L-g4-1
...
# example
# power.cra4r3(rho4=.05, rho3=.15, rho2=.15, omega4=.50, n=10, J=4, K=4, L=20)


# gm = grid multiplier: default 2
# ncase = numer of fixed samples in the output
# constrain = "power", or "mdes" :  default "power"
mrss.cra4r3 <- function(mdes=.25, power=.80, alpha=.05, two.tail=TRUE,
  ...
  if(is.null(L)){
    ...
    T2 = abs(qt(power,df))
    M <- ifelse(power>=.5,T1+T2,T1-T2)
    K <- (rho3*(1-R32) + rho2*(1-R22)/J + (1-rho4-rho3-rho2)*(1-R12)/(J*n)) /
         (P*(1-P)*(L*(mdes/M)^2 - rho4*omega4*(1-RT42)))
    K <- round(K)
  }else if(is.null(J)){
    df = L-g4-1
    T1 <- ifelse(two.tail==TRUE,abs(qt(alpha/2,df)),abs(qt(alpha,df)))
    ...
    J <- (rho2*(1-R22) + (1-rho4-rho3-rho2)*(1-R12)/n) /
         (P*(1-P)*K*(L*(mdes/M)^2 - rho4*omega4*(1-RT42)) - rho3*(1-R32))
    J <- round(J)
  }else if(is.null(n)){
    df = L-g4-1
    T1 <- ifelse(two.tail==TRUE,abs(qt(alpha/2,df)),abs(qt(alpha,df)))
    ...
    n <- (1-rho4-rho3-rho2)*(1-R12) /
         (P*(1-P)*J*K*(L*(mdes/M)^2 - rho4*omega4*(1-RT42)) - J*rho3*(1-R32) -  rho2*(1-R22))
    n <- round(n)
  }
...
# example
# mrss.cra4r3(rho4=.05, rho3=.15, rho2=.15, omega4=.50, n=10, J=2, K=10)
# mrss.cra4r3(rho4=.05, rho3=.15, rho2=.15, omega4=.50, n=10, J=2, L=18)
# mrss.cra4r3(rho4=.05, rho3=.15, rho2=.15, omega4=.50, n=10, K=10, L=18)
# mrss.cra4r3(rho4=.05, rho3=.15, rho2=.15, omega4=.50, J=2, K=10, L=18)
...
optimal.cra4r3 <- function(cn, cJ, cK, cL, cost=NULL, n=NULL, J=NULL, K=NULL, L=NULL,
...
# optimal.cra4r3(cn=1, cJ=10, cK=100, cL=1000, cost=75600, constrain="cost",
#                rho4=.10, rho3=.15, rho2=.20, omega4=.50)

# optimal sample given per unit costs and power 
# optimal.cra4r3(cn=1, cJ=10, cK=100, cL=1000, power=.80, constrain="power",
#                rho4=.10, rho3=.15, rho2=.20, omega4=.50)

# optimal sample given per unit costs and mdes
# optimal.cra4r3(cn=1, cJ=10, cK=100, cL=1000, power=.80, constrain="mdes", 
#                rho4=.10, rho3=.15, rho2=.20, omega4=.50)

# conditional optimal sample (fixed n=10, and J=2) given total cost
# optimal.cra4r3(cn=1, cJ=10, cK=100, cL=1000, n=10, J=2, cost=75600, constrain="cost",
#                rho4=.10, rho3=.15, rho2=.20, omega4=.50)

# conditional optimal sample (fixed n=10, and J=2) given per unit costs and power 
# optimal.cra4r3(cn=1, cJ=10, cK=100, cL=1000, n=10, J=2, power=.80, constrain="power",
#                rho4=.10, rho3=.15, rho2=.20, omega4=.50)

# conditional optimal sample (fixed n=10, and J=2) given per unit costs and mdes
# optimal.cra4r3(cn=1, cJ=10, cK=100, cL=1000, n=10, J=2, power=.80, constrain="mdes", 
#                rho4=.10, rho3=.15, rho2=.20, omega4=.50)
