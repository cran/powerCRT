mdes.cra2r2 <- function(power=.80, alpha=.05, two.tail=TRUE,
  ...
  mdes = M*sqrt(rho2*(1-R22)/(P*(1-P)*J) +
...
power.cra2r2 <- function(mdes=.25, alpha=.05, two.tail=TRUE,
                         ...
                         n, J, ...){
  df = J-g2-2
  lamda <- mdes/sqrt(rho2*(1-R22)/(P*(1-P)*J) +
                    (1-rho2)*(1-R12)/(P*(1-P)*J*n))
...
mrss.cra2r2 <- function(mdes=.25, power=.80, alpha=.05, two.tail=TRUE,
  ...
  eq.mdes <- function(nJ){
    ...
    T2 = abs(qt(power,df))
    M <- ifelse(power>=.5,T1+T2,T1-T2)
    mdes1 = M*sqrt(rho2*(1-R22)/(P*(1-P)*J) +
                  (1-rho2)*(1-R12)/(P*(1-P)*J*n))
    mdes0 <-mdes1-mdes
    return(mdes0)
  }
  ...
  if(is.null(J)){
    ...
    T2 = abs(qt(power,df))
    M <- ifelse(power>=.5,T1+T2,T1-T2)
    n <- (1-rho2)*(1-R12)/(P*(1-P)*J*(mdes/M)^2 - rho2*(1-R22))
    n <- round(n)
  }
...
optimal.cra2r2 <- function(cn, cJ, cost=NULL, n=NULL, J=NULL,
...
# optimal.cra2r2(cn=1, cJ=10, power=.80, constrain="power", rho2=.20)

# optimal sample given per unit costs and mdes
# optimal.cra2r2(cn=1, cJ=10, power=.80, constrain="mdes", rho2=.20)

# conditional optimal sample (fixed n=10) given total cost
# optimal.cra2r2(cn=1, cJ=10, n=10, cost=560, constrain="cost", rho2=.20)

# conditional optimal sample (fixed n=10) given per unit costs and power 
# optimal.cra2r2(cn=1, cJ=10, n=10, constrain="power", rho2=.20)

# conditional optimal sample (fixed n=10) given per unit costs and mdes
# optimal.cra2r2(cn=1, cJ=10, n=10, constrain="mdes", rho2=.20)
