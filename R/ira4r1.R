mdes.ira4r1 <- function(power=.80, alpha=.05, two.tail=TRUE, 
  ...
  par <- list(power=power, alpha=alpha, two.tail=two.tail,
              ...
              P=P, g4=g4, R12=R12, RT22=RT22, RT32=RT32, RT42=RT42,
              n=n, J=J, K=K, L=L)
  return(list(fun=fun,par=par,df=df,M=M,mdes=round(MLU,digits=3)))
}
# example
# mdes.ira4r1(rho4=.05, rho3=.15, rho2=.15, omega4=.50, omega3=.50, omega2=.50, n=10, J=4, L=27, K=4)


power.ira4r1 <- function(mdes=.25, alpha=.05, two.tail=TRUE,
                         rho2, rho3, rho4, omega2, omega3, omega4,
                         P=.50, R12=0, RT22=0, RT32=0, RT42=0, g4=0,
                         n, J, K, L, ...){
  df = L-g4-1
...
# example
# power.ira4r1(rho4=.05, rho3=.15, rho2=.15, omega4=.50, omega3=.50, omega2=.50, n=10, J=4, L=27, K=4)

# gm = grid multiplier: default 2
# ncase = numer of fixed samples in the output
# constrain = "power", or "mdes" :  default "power"
mrss.ira4r1 <- function(mdes=.25, power=.80, alpha=.05, two.tail=TRUE,
  ...
  eq.power <- function(nJKL){
    ...
    lamda <- (mdes/sqrt(rho4*omega4*(1-RT42)/L +
                          rho3*omega3*(1-RT32)/(K*L) +
                          rho2*omega2*(1-RT22)/(J*K*L) +
                          (1-rho4-rho3-rho2)*(1-R12)/(P*(1-P)*J*K*L*n)))[[1]]
  ...
  if(is.null(L)){
    ...
    while(i<=100 & conv==FALSE){
      ...
      L1 <- (M/mdes)^2 * (rho4*omega4*(1-RT42) +
                            ...
                            rho2*omega2*(1-RT22)/(K*J) +
                            (1-rho4-rho3-rho2)*(1-R12)/(P*(1-P)*K*J*n))
    ...
    T2 = abs(qt(power,df))
    M <- ifelse(power>=.5,T1+T2,T1-T2)
    K = (rho3*omega3*(1-RT32) + rho2*omega2*(1-RT22)/J + (1-rho4-rho3-rho2)*(1-R12)/(P*(1-P)*J*n))/
    ...
    T2 = abs(qt(power,df))
    M <- ifelse(power>=.5,T1+T2,T1-T2)
    J = (rho2*omega2*(1-RT22) + (1-rho4-rho3-rho2)*(1-R12)/(P*(1-P)*n))/
      ((K*L*(mdes/M)^2) - K*rho4*omega4*(1-RT42)- rho3*omega3*(1-RT32))
    J <- round(J)
  }else if(is.null(n)){
    df = L-g4-1
    T1 <- ifelse(two.tail==TRUE,abs(qt(alpha/2,df)),abs(qt(alpha,df)))
    ...
    n = (1-rho4-rho3-rho2)*(1-R12)/
      (J*P*(1-P)*((K*L*(mdes/M)^2) - K*rho4*omega4*(1-RT42) - rho3*omega3*(1-RT32) - rho2*omega2*(1-RT22)/J))
    n <- round(n)
  }
...
# examples
# mrss.ira4r1(rho4=.05, rho3=.15, rho2=.15, omega4=.50, omega3=.50, omega2=.50, n=10, J=2, K=10)
# mrss.ira4r1(rho4=.05, rho3=.15, rho2=.15, omega4=.50, omega3=.50, omega2=.50, n=10, J=2, L=8)
# mrss.ira4r1(rho4=.05, rho3=.15, rho2=.15, omega4=.50, omega3=.50, omega2=.50, n=10, K=10, L=8)
# mrss.ira4r1(rho4=.05, rho3=.15, rho2=.15, omega4=.50, omega3=.50, omega2=.50, J=2, K=10, L=8)
...
optimal.ira4r1 <- function(cn, cJ, cK, cL, cost=NULL, n=NULL, J=NULL, K=NULL, L=NULL,
...
# optimal.ira4r1(cn=1, cJ=10, cK=100, cL=1000, cost=75600, constrain="cost",
#               rho4=.10, rho3=.15, rho2=.20, omega4=.50, omega3=.50, omega2=.50)

# optimal sample given per unit costs and power 
# optimal.ira4r1(cn=1, cJ=10, cK=100, cL=1000, power=.80, constrain="power",
#               rho4=.10, rho3=.15, rho2=.20, omega4=.50, omega3=.50, omega2=.50)

# optimal sample given per unit costs and mdes
# optimal.ira4r1(cn=1, cJ=10, cK=100, cL=1000, power=.80, constrain="mdes",  
#               rho4=.10, rho3=.15, rho2=.20, omega4=.50, omega3=.50, omega2=.50)

# conditional optimal sample (fixed n=10, and J=2) given total cost
# optimal.ira4r1(cn=1, cJ=10, cK=100, cL=1000, n=10, J=2, cost=75600, constrain="cost",
#               rho4=.10, rho3=.15, rho2=.20, omega4=.50, omega3=.50, omega2=.50)

# conditional optimal sample (fixed n=10, and J=2) given per unit costs and power 
# optimal.ira4r1(cn=1, cJ=10, cK=100, cL=1000, n=10, J=2, power=.80, constrain="power",
#              rho4=.10, rho3=.15, rho2=.20, omega4=.50, omega3=.50, omega2=.50)

# conditional optimal sample (fixed n=10, and J=2) given per unit costs and mdes
# optimal.ira4r1(cn=1, cJ=10, cK=100, cL=1000, n=10, J=2, power=.80, constrain="mdes",
#               rho4=.10, rho3=.15, rho2=.20, omega4=.50, omega3=.50, omega2=.50)
